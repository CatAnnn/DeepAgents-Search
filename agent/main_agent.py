...
project_root_path = Path(__file__).parents[1].resolve() # 绝对路径 解析路径标识以及软连接
# project_root_path = Path(__file__).parents[1].absolute() # 绝对 不会进行解析真实地址（参考桌面的快捷方式）
# main_agent.invoke()
# main_agent.stream()
...
async def run_deep_agent(task_query,session_id):
    ...
    # 1. 准备工作 【1. session_dir（前端） 2. relative_session_dir (大模型) 3. 上传的文件拼接上传文件专属提示词】
    # project_root_path / output / session_session_id(uuid) 其中,uuid: 唯一标识符，每个会话的唯一标识
    # 当前会话存储生成文件的专属文件夹
    ...
    if updated_dir_path.exists():
        # 有
        files = [ f.name  for f in updated_dir_path.iterdir()  if f.is_file()]# 将获取到的文件名都放到files列表中
